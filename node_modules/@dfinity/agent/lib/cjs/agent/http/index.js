...
class HttpAgent {
    ...
    async prepareCallRequest(canisterId, options, sender) {
    ...
    async submitRequest(req) {
        ...
        console.log(Buffer.from(await (0, request_id_1.requestIdOf)(req.body.content)).toString('hex'));
        const ecid = req.body.content.canister_id;
        const request = this._requestAndRetry(() => this._fetch('' + new URL(`/api/v2/canister/${ecid.toText()}/call`, this._host), Object.assign(Object.assign({}, req.request), { body })));
        const [response, requestId] = await Promise.all([request, (0, request_id_1.requestIdOf)(req.body.content)]);
        return {
            requestId,
            response: {
                ok: response.ok,
    ...
    async call(canisterId, options, identity) {
        ...
        console.log("transformedRequest: ", transformedRequest.body);
